...
set -e
# ========================================================================================
# Aura Protocol - v19.0 (Graduate Edition)
# 以 v10 稳定内核为绝对基石，融合新架构能力，经三重审判最终验证
# ========================================================================================
SCRIPT_VERSION="19.0 (Graduate Edition)"
...
setup_optimize_ip_cronjob() {
    ...
    local current_cron_entry; current_cron_entry=$(echo "$cron_list" | grep -F "${cron_command_signature}")
    if [[ -n "$current_cron_entry" ]]; then
        info "检测到已存在的定时任务:"; echo -e "${YELLOW}${current_cron_entry}${NC}"
        ask "您想 [删除(d)] 还是 [修改(m)]？(d/m/N): "; read -r action
        case "$action" in d|D) (echo "$cron_list" | grep -vF "${cron_command_signature}") | sudo crontab -; rm -f "$LOGROTATE_CONFIG_FILE"; info "定时任务已删除。"; return ;; m|M) info "请重新设置。" ;; *) info "操作取消。"; return ;; esac
    fi
    ...
    (echo "$cron_list" | grep -vF "${cron_command_signature}"; echo "${cron_command}") | sudo crontab -
...
uninstall_aura() {
    ...
    info "-> 清理临时文件和依赖..."; rm -f /root/install.sh /tmp/cloudflared.deb /tmp/aura-server.zip /tmp/gh.deb; apt-get purge -y gh cloudflared htop >/dev/null; apt-get autoremove -y >/dev/null
