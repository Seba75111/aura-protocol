...
# ========================================================================================
# Aura Protocol - v12.7 (绝对稳定版)
# 最终修复版，包含所有功能和最健壮的安装与卸载逻辑
# ========================================================================================

# --- 脚本信息与颜色定义 ---
SCRIPT_VERSION="12.7 (绝对稳定版)"
...
check_root() { if [[ $EUID -ne 0 ]]; then error "本脚本需要以 root 用户或 sudo 权限运行。"; fi }
...
function install_aura() {
    ...
    apt-get install -y curl jq git unzip net-tools iproute2 cron logrotate dnsutils bc >/dev/null || error "依赖安装失败。"
    ...
    mv "${TEMP_EXTRACT_PATH}" "${INSTALL_PATH}" || { error "重命名 Aura Server 失败。"; rm -f "${TEMP_ZIP_PATH}" "${TEMP_EXTRACT_PATH}"; }
...
EOF
    ...
    if [[ ! "$enable_cloud_optimizer" =~ ^[nN]$ ]]; then
        ...
        source <(curl -sL -H "Authorization: token ${GITHUB_PAT_FOR_SCRIPT}" "${remote_script_url}") || error "无法加载远程脚本。"
        if ! command -v setup_cloud_hunter &> /dev/null; then error "远程脚本中未找到 setup_cloud_hunter 函数。"; fi
...
function _sync_ip_from_cloud() {
    ...
    source "$CF_META_FILE" 2>/dev/null || true
    if [[ -z "$FAST_OPTIMIZE_PREFIX" || "$FAST_OPTIMIZE_PREFIX" == "null" ]]; then warn "优选IP前缀未设置，跳过。"; return 0; fi
    info "正在执行本地终端制导优选 IP..."
    local PING_CMD="ping"
    if ping -6 -c 1 -W 1 google.com &>/dev/null; then info "检测到 IPv6 网络环境。"; PING_CMD="ping -6"; else info "使用 IPv4 网络环境。"; fi
    local FLEET_MEMBERS=5; local CANDIDATE_IPS=()
...
function optimize_ip_cron() {
    ( echo "$(date '+%Y-%m-%d %H:%M:%S'): 开始执行本地终端制导..."; if [ -f "$CF_META_FILE" ]; then source "$CF_META_FILE" 2>/dev/null || true; _sync_ip_from_cloud; fi ) >> "${OPTIMIZER_LOG_FILE}" 2>&1
}

function setup_logrotate() {
    mkdir -p "$(dirname "$OPTIMIZER_LOG_FILE")"; touch "$OPTIMIZER_LOG_FILE"
    cat > "$LOGROTATE_CONFIG_FILE" <<EOF
${OPTIMIZER_LOG_FILE} { daily; rotate 7; size 10M; compress; delaycompress; missingok; notifempty; }
EOF
}

function setup_optimize_ip_cronjob() {
    info "正在为您设置默认的本地优选IP定时任务 (每天凌晨4点)..."
    mkdir -p "${AURA_OPERATIONS_DIR}"; cp "$0" "${AURA_OPERATIONS_DIR}/run.sh"
    local cron_command="0 4 * * * root bash ${AURA_OPERATIONS_DIR}/run.sh optimize_ip_cron"
...
function uninstall_aura() {
    ...
    if [[ "$confirm" != "yes" ]]; then info "操作已取消。"; return; fi
